...
export async function runAudit(auditType, userInput, imageBase64 = null, userContext = null) {
  ...
  const contextBlock = userContext
    ? `\n\nIMPORTANT CONTEXT: This creator is in the ${userContext.niche} space, targeting ${userContext.audience}, with a goal to ${userContext.goal}. Make your feedback specific to this context. Reference their niche in at least one action item.`
    : ''
  ...
  const systemPrompt = `You are Blair Richards — a direct, warm, no-fluff social media strategist who built $3M+ in revenue in 2 years with zero ad spend. You speak to women building personal brands and digital businesses. You have coaching call energy — encouraging but you never sugarcoat.
...
CRITICAL: You MUST respond with ONLY valid JSON. No markdown, no code fences, no extra text. The "action_items" field MUST be a JSON array of 5 strings — do NOT use unescaped quotes or apostrophes inside strings (use unicode \\u0027 for apostrophes if needed). Use this exact format:
{
  ...
  "summary": "<2-3 sentences in Blair's voice summarizing the audit>",
  "action_items": [
  ...
  try {
    ...
    try {
      ...
      // Try to fix by finding "action_items" and ensuring it's a proper array
      // Replace the action_items value with a reconstructed array
      const actionItemsMatch = jsonStr.match(/"action_items"\s*:\s*\[?([\s\S]*?)\]?\s*\}$/)
      if (actionItemsMatch) {
        // Extract everything before action_items
        const beforeAI = jsonStr.substring(0, jsonStr.indexOf('"action_items"'))
        // Split action items by the pattern of ","  (quote-comma-quote between items)
        const aiRaw = actionItemsMatch[1]
        // Try to parse individual strings - split on the pattern of ending quote + comma + starting quote
        const items = []
        const itemMatches = aiRaw.match(/"([^"]*(?:\\.[^"]*)*)"/g)
        if (itemMatches) {
          itemMatches.forEach(m => {
            const clean = m.slice(1, -1) // remove surrounding quotes
            if (clean.length > 10) items.push(clean) // skip tiny fragments
          })
        }
        if (items.length >= 3) {
